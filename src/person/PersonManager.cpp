...
Person* PersonManager::copyPerson(const Person* other) {
    ...
    return other->clone(); // Safely copies exact Student/Lecturer types without slicing [cite: 317-318]
}
...
vector<Person*> PersonManager::getAllPeopleCollection() const {
    ...
    return out; // Non-owning wrapper list [cite: 339-340]
}
...
// FIXED: Properly captures the 'doomed' pointer and advances the list head!
