...
bool FacultyManager::deleteFaculty(Faculty* faculty) {
    ...
    // 1) Clear every course run by this faculty [cite: 534]
    if (courseManager) {
        courseManager->removeFacultyFromCourseData(faculty);
    }

    // 2) Detach all owned buildings [cite: 535]
    for (Building* b : faculty->data.buildings) {
        if (b) b->ownedBy = nullptr;
    }

    // 3) Detach employed persons (Simply reset back-pointers, no allocation tracking needed!) 
...
void FacultyManager::addStudentToFaculty(Student* s, Faculty* f) {
    ...
    s->employedBy = f; // Set the base Person back-pointer seamlessly!
}

void FacultyManager::addLecturerToFaculty(Lecturer* l, Faculty* f) {
    if (!l || !f) return;
    f->data.lecturers.push_back(l);
    l->employedBy = f; // Set the base Person back-pointer seamlessly!
}
...
bool FacultyManager::removePersonFromFaculty(Person* p, Faculty* f) {
    ...
    // Check student vector: Upcasting allows clean comparison directly [cite: 549]
    ...
    // Check lecturer vector [cite: 549]
