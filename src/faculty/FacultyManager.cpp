...
bool FacultyManager::deleteFaculty(Faculty* faculty) {
    ...
    // 2) Detach all owned buildings safely using class setter methods
    // FIX: Replaced direct b->ownedBy pointer assignments with b->setFaculty() calls
    for (Building* b : faculty->data.buildings) {
        if (b) b->setFaculty(nullptr);
    }
...
void FacultyManager::addBuildingToFaculty(Building* b, Faculty* f) {
    ...
    // FIX: Replaced b->ownedBy verification with b->getFaculty()
    if (b->getFaculty() == f) return;

    // FIX: Replaced b->ownedBy verification with b->getFaculty()
    if (b->getFaculty() != nullptr) {
        removeBuildingFromFaculty(b, b->getFaculty());
    }

    f->data.buildings.push_back(b);
    // FIX: Replaced b->ownedBy assignment with b->setFaculty()
    b->setFaculty(f);
}
...
bool FacultyManager::removeBuildingFromFaculty(Building* b, Faculty* f) {
    ...
    for (auto it = f->data.buildings.begin(); it != f->data.buildings.end(); ++it) {
        if (*it == b) {
            ...
            // FIX: Replaced b->ownedBy = nullptr with b->setFaculty(nullptr)
            b->setFaculty(nullptr);
            return true;
        }
    }
    return false;
